# 🇧🇷 SolidSign API - Exemplo de Integração: Assinatura PAdES (PDF) com PKCS#12 (certificado pré-importado) (Node.js (Fastify))
...
- Fastify / Node.js 18+ (`npm install && cp .env.example .env`)
...
O serviço sobe em `http://localhost:3088`.
...
# 🇬🇧 SolidSign API - Integration Example: PAdES (PDF) Signing with PKCS#12 (pre-imported certificate) (Node.js (Fastify))
...
The service starts on `http://localhost:3088`.
